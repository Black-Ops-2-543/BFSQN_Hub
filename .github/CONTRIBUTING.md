...
2. Create a new fork of this repository so that you can edit your own copy.
  ...
  - Name your fork something reasonable. It should follow the form `BFSQN_<name>`. The `<name>` should be no more than two words and should have some relation to the spoke.
3. Load into your coding environment. If you use (the preferred) Visual Studio Code, [this tutorial](https://code.visualstudio.com/docs/sourcecontrol/overview) does a good job at teaching how to use GitHub.
...
Generally, it's difficult to really instruct you on how to build your mod. As general practice, please make all your `Cfg<class>` classes in individual files that you then `#include` into `config.cpp`. Having `CfgPatches` still inside `config.cpp` is accepted and expected.

# Last Steps

Once you're finished with developing your spoke, you'll need to submit a **pull request** through GitHub so that your fork can be merged into the main branch. The owner of this Github Repository (@blackops2543 on Discord) will look over your spoke, ensure everything is up to standard and using BFSQN MH convention, and then finally pull into the `main` branch if all is well. Expect an update on the Steam Workshop shortly after that.
